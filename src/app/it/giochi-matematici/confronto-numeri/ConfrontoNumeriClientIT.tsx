...
export default function ConfrontoNumeriClientIT() {
  ...
  const getRankInfo = (finalScore: number) => {
    if (finalScore <= 5) return { rank: 'Principiante del Confronto', emoji: '🌱', color: 'text-green-600' }
    if (finalScore <= 10) return { rank: 'Esperto del Confronto', emoji: '⚡', color: 'text-yellow-600' }
    if (finalScore <= 15) return { rank: 'Maestro del Confronto', emoji: '🎯', color: 'text-blue-600' }
    return { rank: 'Genio del Confronto', emoji: '🧠', color: 'text-red-600' }
  }
  ...
  return (
    <GameLayout
      ...
      introduction="Ti mostreremo due numeri e dovrai determinare la relazione tra loro. Hai 45 secondi per risolverne quante più possibile!"
      timeLeft={timeLeft}
      score={score}
      onStart={startGame}
      onReset={resetGame}
      ...
      currentGamePath="/it/giochi-matematici/confronto-numeri"
    >
      {gameState === 'waiting' && (
