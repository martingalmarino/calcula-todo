...
const generateProblem = (): EquationProblem => {
  ...
  if (operation === '+') {
    ...
    const x = Math.floor(Math.random() * 10) + 1
    equation = `${a} × ${b} = x × ${x}`
    correctAnswer = Math.round((a * b) / x)
  } else {
    const a = Math.floor(Math.random() * 20) + 10
...
export default function EquazioniExpressClientIT() {
  ...
  const getRankInfo = (finalScore: number) => {
    if (finalScore <= 3) return { rank: 'Principiante delle Equazioni', emoji: '🌱', color: 'text-green-600' }
    if (finalScore <= 6) return { rank: 'Esperto delle Equazioni', emoji: '⚡', color: 'text-yellow-600' }
    if (finalScore <= 9) return { rank: 'Maestro delle Equazioni', emoji: '🎯', color: 'text-blue-600' }
    return { rank: 'Genio delle Equazioni', emoji: '🧠', color: 'text-red-600' }
  }
  ...
  return (
    <GameLayoutIT
      ...
      introduction="Ti mostreremo equazioni semplici e dovrai trovare il valore di x. Hai 50 secondi per risolverne quante più possibile!"
      timeLeft={timeLeft}
      score={score}
      onStart={startGame}
      onReset={resetGame}
      ...
      currentGamePath="/it/giochi-matematici/equazioni-express"
    >
      {gameState === 'waiting' && (
        <div className="text-center space-y-6">
          ...
          <p className="text-gray-600 text-lg max-w-md mx-auto">
            Ti mostreremo equazioni semplici e dovrai trovare il valore di x. 
